...
class Controller {
    ...
    public function invoke() {
        switch ($this->action) {
            ...
            case 'Home':
                ...
                break;
            case 'Registration':
                $this->ProcessShowRegistrationPage();
                break;
            case 'Register':
    ...
    private function processShowRegistrationPage() {
        $reg_error_message = '';
    ...
    private function processRegister(){
        ...
        $username = filter_input(INPUT_POST, 'username');
        $password = filter_input(INPUT_POST, 'password');
        if ($this->db->registerNewUser($username, $password)) {
            $_SESSION['register_new_user'] = true;
            $_SESSION['username'] = $username;
            header("Location: .?action=Show Tasks");
        } else {
            $reg_error_message = $this->db->getRegErrorMessage();
            include('./view/registration.php');
        }
    }
